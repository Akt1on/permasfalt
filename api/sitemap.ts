...
export default async function handler(req: any, res: any) {
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      const [s, pr, po] = await Promise.all([
        sb.from("services").select("slug,updated_at").eq("is_active", true),
        sb.from("projects").select("slug,updated_at").eq("is_active", true),
        sb.from("posts").select("slug,updated_at").eq("is_published", true),
      ]);
      services = s.data ?? [];
      projects = pr.data ?? [];
      posts = po.data ?? [];
    } catch {
      // proceed with static only
    }
  }
